...
import { useAuth, addOwnedCard, getOwnedCards } from "../lib/hooks";
import {
...
export default function GameContainer() {
  ...
  useEffect(() => {
    ...
    getOwnedCards(token)
      .then((response) => {
        ...
        const dbCards = response.cards.map((card: any) => ({
          id: card._id?.toString() ?? card.cardId,
          name: card.cardName,
        ...
        saveCollection(dbCards);
      })
      .catch((error) => {
        console.error('Failed to load owned cards from MongoDB:', error);
      });
  }, [token, saveCollection]);
  ...
  function handleKeepCards(newCards: OwnedCard[], source: "regular" | "deluxe" | "wkey") {
    ...
    Promise.all(
      ...
      console.error('Failed to save owned cards to MongoDB:', error);
    });
  }
    ...
    <div className="min-h-screen flex bg-zinc-950 text-white">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="shrink-0 h-14 border-b border-zinc-800 bg-zinc-950 flex items-center justify-between px-6 gap-2">
          ...
          <button
            onClick={logout}
            className="px-4 py-1.5 rounded-lg text-sm font-medium text-red-400 border border-red-700 hover:bg-red-950 hover:text-red-300 transition-all"
          >
            Logout
          </button>
        </header>
